            ...
            <div class="col-12">
                ...
                >
                    <div class="mb-3">
                        ...
                        <input value="{{ old('title') ?? $post->title }}" type="text" id="title" name="title" class="form-control @error('title') is-invalid @enderror">
                        @error('title')
                    ...
                    <div class="mb-3">
                        ...
                        <input value="{{ old('slug') ?? $post->slug }}" type="text" id="slug" name="slug" class="form-control @error('slug') is-invalid @enderror">
                        @error('slug')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <textarea
                            ...
                            rows="5"
                            required
                        >{{ old('meta_description') ? old('meta_description') : $post->meta_description }}</textarea>
                    ...
                    <div class="mb-3">
                        <img src="{{ Illuminate\Support\Facades\Storage::url($post->src) }}" alt="{{ $post->title }}">
                    </div>
                    <div class="mb-3">
                        <label for="text" class="form-label">Содержание</label>
                        <div id="edit-post-quill-editor" style="min-height: 400px;"></div>
                        <input type="hidden" name="plain_text">
                        <textarea class="form-control" id="text" name="text" rows="3" style="display: none;">{{ $post->text }}</textarea>
                        @error('text')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="row">
                        <div class="col-6">
                            <div class="form-check">
                                <input
                                    @if (boolval($post->published_at)) checked @endif
                                    class="form-check-input @error('publish') is-invalid @enderror"
                                    type="checkbox"
                                    id="publish"
                                    name="publish"
                                >
